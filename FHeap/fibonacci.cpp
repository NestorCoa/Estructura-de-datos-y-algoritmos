...
void fibonacci<T>::actualizar(){
	auto it=root.begin();
	std::vector<nodo<T>*> temp(ceil(log(_size))+1); 
	while(it!=root.end()){
		merge(temp,*it);
		++it;
	}
...
template<class T>
void fibonacci<T>::merge(std::vector<nodo<T>*> &v,nodo<T> *p){
	int po=p->grade;
	if(v[po]==0){
		v[po]=p;
	}	
...
template<class T>
nodo<T>* fibonacci<T>::_union(nodo<T> *p,nodo<T> *q){
	if(p->date > q->date){
		q->grade++;
		p->father=q;
...
template<class T>
bool fibonacci<T>::findroot(T d,std::list<nodo<T>*> &son,nodo<T> **&p){
	auto iter=son.begin();
	while(iter!=son.end()){
		if((*iter)->date==d){
			p=&(*iter);
...
template<class T>
void fibonacci<T>::decresekey(nodo<T> **p){
	auto q=min->date-1;
	while((*p)->father){
...
template<class T>
void fibonacci<T>::print(std::list<nodo<T>*> son,int cont,std::ofstream &file){
	auto it=son.begin();
	while(it!=son.end()){
		file<<(*it)->father->date<<" ->";
		file<<(*it)->date<<";"<<std::endl;
